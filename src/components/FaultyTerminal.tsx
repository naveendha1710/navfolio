...
function FaultyTerminal({
  ...
  // WebGL Renderer Setup (Mounted once)
  useEffect(() => {
    const ctn = containerRef.current;
    if (!ctn) return;
    ...
    try {
      ...
      console.warn('[FaultyTerminal] ❌ WebGL initialize error:', err);
      return;
    }
    ...
    const initialW = ctn.offsetWidth || window.innerWidth || 800;
    const initialH = ctn.offsetHeight || window.innerHeight || 600;
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        iTime: { value: 0 },
        iResolution: {
          value: new Float32Array([gl.canvas.width || initialW, gl.canvas.height || initialH, initialAspect])
        },
        uScale: { value: scale },
    ...
    function resize() {
      if (!ctn || !renderer || !programRef.current) return;
      ...
      resUniform[0] = gl.canvas.width || width;
      resUniform[1] = gl.canvas.height || height;
      resUniform[2] = aspect;
    }
    ...
    return () => {
      console.log('[FaultyTerminal] Cleanup — destroying OGL context');
      cancelAnimationFrame(rafRef.current);
      resizeObserver.disconnect();
      // Always attempt removal — avoids leak if mouseReact changes between renders
      window.removeEventListener('mousemove', handleMouseMove);
      if (gl.canvas.parentElement === ctn) ctn.removeChild(gl.canvas);
      loadAnimationStartRef.current = 0;
    };
  }, [dpr, handleMouseMove, mouseReact]); // mouseReact added to re-bind correctly when toggled

  // Dynamic Uniform Updates (Re-apply immediately on HMR or Prop Change)
  useEffect(() => {
    ...
    program.uniforms.uTint.value = new Float32Array(tintVec);
...
// Memo-wrap — FaultyTerminal props are stable from App.tsx, preventing expensive WebGL re-init
export default memo(FaultyTerminal as React.FC<FaultyTerminalProps>);
