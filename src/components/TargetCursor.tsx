...
export default function TargetCursor({
  ...
  // Store resumeTimeout in a ref so it can be cleared on unmount
  const resumeTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const [isFirstPage, setIsFirstPage] = useState(true);

  // Monitor window scroll to only activate TargetCursor on the first page
  useEffect(() => {
    const handleScroll = () => {
      const threshold = window.innerHeight * 0.85;
      const onFirstPage = window.scrollY < threshold;
      isFirstPageRef.current = onFirstPage;
      setIsFirstPage(onFirstPage);

      if (!onFirstPage && cursorRef.current) {
  ...
  const moveCursor = useCallback((x: number, y: number) => {
    if (!cursorRef.current || !isFirstPageRef.current) return;
  ...
  useEffect(() => {
    ...
    const tickerFn = () => {
      if (!targetCornerPositionsRef.current || !cursorRef.current || !cornersRef.current || !isFirstPageRef.current) {
        return;
      }
    ...
    const mouseDownHandler = () => {
      if (!dotRef.current || !isFirstPageRef.current) return;
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      // Reveal TargetCursor only if on first page
    ...
    return () => {
      ...
      // Clear pending resume timeout to prevent post-unmount state updates
      if (resumeTimeoutRef.current) {
        clearTimeout(resumeTimeoutRef.current);
        resumeTimeoutRef.current = null;
      }
