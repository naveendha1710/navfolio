...
export default function ScrollExpand({
  ...
  const applyProgress = useCallback((p: number) => {
    ...
    if (overlayRef.current) {
      // 100% VISIBLE (opacity = 1) until black stage fully covers screen (p: 0 -> 1)
      // Fades out (1 -> 0) when scrolling past full screen (p > 1)
      // Fully reversible when de-scrolling / scrolling back up!
      let opacityVal = 1;
      if (p < 0) {
        opacityVal = clamp(1 + p * 3, 0, 1);
      } else if (p > 1) {
        opacityVal = clamp(1 - (p - 1) * 3, 0, 1);
      } else {
        opacityVal = 1;
      }

      overlayRef.current.style.opacity = `${opacityVal}`;
      overlayRef.current.style.transform = `translate3d(0, ${-20 * (1 - opacityVal)}px, 0)`;
    }
  }, []);
  ...
  const media =
    mediaType === 'video' ? (
      ...
      <div
        ref={mediaRef}
        className="absolute inset-0 w-full h-full bg-[#05070a] origin-center select-none [will-change:transform]"
      >
        {src ? (
          <img
      ...
      <div ref={trackRef} className="relative w-full">
        <div ref={stageRef} className="sticky top-0 w-full overflow-hidden [--se-title-size:4rem]">
          ...
          >
            ...
            <div
              ref={scrimRef}
              className="absolute inset-0 opacity-0 pointer-events-none bg-[linear-gradient(to_top,rgba(0,0,0,0.85),rgba(0,0,0,0.3)_45%,rgba(0,0,0,0.5))]"
            />
            {children ? (
              <div
                ref={overlayRef}
                className="absolute inset-0 flex flex-col items-center justify-center text-center p-[6%] z-10 pointer-events-auto [will-change:opacity,transform]"
              >
                {children}
              </div>
            ) : null}
